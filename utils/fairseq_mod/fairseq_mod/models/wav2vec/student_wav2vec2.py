...
@register_model("student_wav2vec2")
class StudentWav2Vec2Model(BaseFairseqModel):
    ...
    def forward(self, source, padding_mask=None, mask=True, features_only=False, temperature=1):
        ...
        if features_only:
            return {"x": x, "padding_mask": padding_mask, "loss":{"features_pen": features_pen, "student_loss":0},
                    "logits": logits, "prob": token_prob, "log_prob": token_log_prob, "cnn_time":cnn_time, "encoder_time":encoder_time,
                    "between_cnn_encoder_time":between_cnn_encoder_time, "conv_time":conv_time, "max_pool_time":max_pool_time}
        ...
        result = {"x": x, "padding_mask": padding_mask, "features_pen": features_pen, 'mask_indices': mask_indices,
                  "student_feature":student_feature, "student_target":student_target, "student_negs": negs}
    ...
    def init_proj_layer_to_decoder(self, proj_layer_weight, proj_layer_bias):
        # Initialize the projection layer before passing to the decoder
        # This layer is equivalent to self.proj in class Wav2VecEncoder, at wav2vec2_asr.py
    ...
    @classmethod
    def create_student_model(cls,
                             target_dict,
                             fairseq_pretrained_model_path,
                             num_trans_layer_student_model,
                             num_trans_layer_student_init_model,
                             student_init_model_path,
                             student_init_model_type,
                             student_trans_layer_init_method):
        """
        ...
        """
        w2v = torch.load(fairseq_pretrained_model_path) # use the arg set up from fairseq's pre-trained model to initialize the student model
        w2v["args"].w2v_args.encoder_layers = num_trans_layer_student_model # student model has fewer transformer layers
        #w2v["args"].w2v_args.encoder_embed_dim = 1024
        ...
        if student_init_model_type == "fairseq_pretrained":
           student_init_model_state = w2v["model"]
            ...
            student_init_model = torch.load(student_init_model_path, map_location=torch.device('cpu')) # e.g. /home/knowledge_distillation/speech-processing/train_on_train_clean_100_exp9/checkpoints/student-epoch=018-student_wer_dev_clean=0.05934.ckpt
        ...
        # Initialize every layer of the student model, except the transformers
        for name, param in student_init_model_state.items():
            ...
            if name_as_list[0] == 'encoder': continue
            ...
            if name_in_s_model in student_model_state:
                ...
                print(name + ' is not in student model state_dict')
        ...
        transformer_parts = [".self_attn.k_proj.weight", ".self_attn.k_proj.bias", ".self_attn.v_proj.weight", ".self_attn.v_proj.bias", ".self_attn.q_proj.weight", ".self_attn.q_proj.bias",
        ...
        print('Finished loading weights into the student model')
...
class ConvFeatureExtractionModel(nn.Module):
    def __init__(
        ...
        # !!! TEMPORARY pooling layer
        self.max_pool_1 = nn.MaxPool1d(3, stride=2)
        self.max_pool_2 = nn.MaxPool1d(2, stride=2)

    def forward(self, x):

        conv_time, max_pool_time = 0, 0

        # BxT -> BxCxT
        x = x.unsqueeze(1)

        if len(self.conv_layers) == 7:
            start_time = time.time()
            for conv in self.conv_layers:
                x = conv(x)
            conv_time = time.time() - start_time
        elif len(self.conv_layers) == 4:
            # TEMPORARY. apply max pooling after some conv layers
            start_time = time.time()
            x = self.conv_layers[0](x)
            conv_time = conv_time + time.time() - start_time

            start_time = time.time()
            x = self.max_pool_1(x)
            max_pool_time = max_pool_time + time.time() - start_time
        ...
        return x, conv_time, max_pool_time
...
@register_model_architecture("student_wav2vec2", "student_wav2vec2")
