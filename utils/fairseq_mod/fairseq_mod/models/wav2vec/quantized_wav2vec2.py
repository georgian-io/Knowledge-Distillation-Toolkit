...
@register_model("quantized_wav2vec2")
class QuantizedWav2Vec2Model(BaseFairseqModel):
    ...
    def forward(self, source, padding_mask=None, mask=True, features_only=False):
        ...
        #features_pen = features.float().pow(2).mean() # Comment this out for now to make quantization work
        features_pen = 0
        ...
        if torch.backends.quantized.engine == 'qnnpack':
            features = features.unsqueeze(0) # need to add dimension here because quantized.linear shrink dimension when batch size is 1
...
class ConvFeatureExtractionModel(nn.Module):
    def __init__(
        ...
        self.gelu = nn.GELU()
        # Quantize and De-quantize before and after Gelu because quantized gelu is not supported yet.
        self.dequantize_gelu_input = torch.quantization.DeQuantStub()
        self.quantize_gelu_output = torch.quantization.QuantStub(qconfig=None)
    ...
    def forward(self, x):
        ...
        for conv in self.conv_layers:
            x = conv(x)
            x = self.dequantize_gelu_input(x)
            x = self.gelu(x)
            x = self.quantize_gelu_output(x)
...
class TransformerEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.pos_conv = nn.Sequential(self.pos_conv, SamePad(args.conv_pos), self.dequantize_gelu_input, nn.GELU(), self.quantize_gelu_output)
        ...
        self.quantize_input = QuantStub()
        self.quantize_add = nn.quantized.FloatFunctional()
    ...
    def extract_features(self, x, padding_mask=None):
        ...
        x = self.quantize_add.add(x, x_conv)
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # De-quantize input to Gelu activation (quantized version of Gelu is not supported yet.)
        self.dequantize_gelu_input = torch.quantization.DeQuantStub()

        # Quantize output from multi-head attention
        self.quantize_mha_output = torch.quantization.QuantStub(qconfig=None)

        # Quantize output from gelu activation
        self.quantize_gelu_output = torch.quantization.QuantStub(qconfig=None)

        # Initialize add modules for quantized tensors
        self.quantize_add1 = nn.quantized.FloatFunctional()
        self.quantize_add2 = nn.quantized.FloatFunctional()
    ...
    def forward(
        ...
        if self.layer_norm_first:
            ...
            x = self.quantize_mha_output(x)
            x = self.dropout1(x)
            x = self.quantize_add1.add(residual, x)
            ...
            x = self.fc2(x)
            x = self.dropout3(x)
            x = self.quantize_add2.add(residual, x)
        else:
            x, attn = self.self_attn(
                query=x,
                key=x,
...
@register_model_architecture("quantized_wav2vec2", "quantized_wav2vec2")
